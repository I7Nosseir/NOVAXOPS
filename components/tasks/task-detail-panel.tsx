...
import {
  ...
  Square, SquareCheck, ListChecks, Bell, BellOff,
  Link2, Timer, Activity, Bookmark, BookmarkCheck, CalendarRange,
  Link, Unlink, ArrowRight, BookTemplate,
} from 'lucide-react'
import { useQuery, useQueryClient } from '@tanstack/react-query'
import { toast } from 'sonner'
import type { Task, AgentType, PipelineStage, Priority, TaskStatus, ContentBriefRequest, ContentBriefData, ChecklistItem, TaskRelation, TaskTimeLog } from '@/lib/types'
...
import { useUpdateTask, useDeleteTask, useCreateTask, useTaskRelations, useCreateTaskRelation, useDeleteTaskRelation, useTaskTimeLogs, useCreateTimeLog, useDeleteTimeLog } from '@/lib/hooks/use-tasks'
...
export function TaskDetailPanel({ task, onClose }: Props) {
  ...
  useEffect(() => {
    if (task) {
      ...
      setDraftEstHours(task.estimated_hours != null ? String(task.estimated_hours) : '')
      setEditingField(null)
      setShowMenu(false)
      setDeleteConfirm(false)
      setShowActivityLog(false)
      setShowRelations(false)
      setShowTimeLogs(false)
    }
  }, [task?.id])
  ...
  const saveChecklist = (next: ChecklistItem[]) => {
    setChecklist(next)
    updateTask.mutate({ id: task.id, checklist: next })
  }
  ...
  // ── Watcher helpers ─────────────────────────────────────────────────────────
  const isWatching = !!authUser && (task.watchers ?? []).includes(authUser.id)

  const toggleWatcher = () => {
    if (!authUser) return
    const current = task.watchers ?? []
    const next = isWatching
      ? current.filter(id => id !== authUser.id)
      : [...current, authUser.id]
    updateTask.mutate({ id: task.id, watchers: next })
  }

  // ── New state for Sprint 2 features ────────────────────────────────────────
  const [draftStartDate, setDraftStartDate]     = useState('')
  const [draftEstHours, setDraftEstHours]       = useState<string>('')
  const [showActivityLog, setShowActivityLog]   = useState(false)
  const [showRelations, setShowRelations]       = useState(false)
  const [showTimeLogs, setShowTimeLogs]         = useState(false)
  const [relSearchQuery, setRelSearchQuery]     = useState('')
  const [relSearchType, setRelSearchType]       = useState<'blocks' | 'related'>('related')
  ...
  const { data: relations } = useTaskRelations(task?.id ?? '')
  ...
  // Fetch all tasks for relation search
  const { data: allTasksForSearch = [] } = useQuery<Pick<Task, 'id' | 'title' | 'pipeline_stage' | 'status'>[]>({
    queryKey: ['tasks-search-light'],
    queryFn: () => fetch('/api/tasks?light=1').then(r => r.json()).catch(() => []),
    staleTime: 30_000,
  })

  // Fetch activity log from audit_log for this task
  const { data: activityLog = [] } = useQuery<{
    id: string; action: string; user_id: string; created_at: string; metadata: Record<string, unknown> | null
  }[]>({
    queryKey: ['audit-log-task', task?.id],
    queryFn: async () => {
      if (!task?.id) return []
    ...
    enabled: !!task?.id && showActivityLog,
    staleTime: 30_000,
  })
  ...
  const handleDuplicate = async () => {
    ...
    try {
      await createTask.mutateAsync({
        ...
        status: 'pending',
        sub_type: task.sub_type ?? null,
        due_date: null,
        tags: task.tags ?? [],
        linked_doc_ids: task.linked_doc_ids ?? [],
      })
  ...
  const handleSaveAsTemplate = async () => {
    if (!task || savingTemplate) return
    setSavingTemplate(true)
    try {
      await updateTask.mutateAsync({ id: task.id, is_template: true })
  ...
  const handleLogTime = async () => {
    const hrs = parseFloat(timeLogHours)
    if (!authUser || isNaN(hrs) || hrs <= 0 || timeLogSaving) return
  ...
  // ── Task relations search ──────────────────────────────────────────────────
  const relSearchResults = relSearchQuery.trim()
    ? allTasksForSearch
        .filter(t => t.id !== task.id &&
          t.title.toLowerCase().includes(relSearchQuery.toLowerCase()) &&
          !(relations?.outgoing ?? []).some(r => r.to_task_id === t.id) &&
          !(relations?.incoming ?? []).some(r => r.from_task_id === t.id))
        .slice(0, 5)
    : []
  ...
  return (
    <>
      ...
      >
              ...
              {showMenu && (
                <div className="absolute right-0 top-full mt-1 w-52 bg-white border border-slate-200 rounded-xl shadow-lg z-10 overflow-hidden">
                  {deleteConfirm ? (
                    ...
                    <div className="py-1">
                      <button
                        onClick={() => { updateTask.mutate({ id: task.id, is_pinned: !task.is_pinned }); setShowMenu(false) }}
                        className="w-full flex items-center gap-2.5 px-3 py-2 text-sm text-slate-700 hover:bg-slate-50 transition-colors"
                      >
                        {task.is_pinned ? <BookmarkCheck className="w-3.5 h-3.5 text-novax" /> : <Bookmark className="w-3.5 h-3.5" />}
        ...
        {/* Blocked-by banner — shown at top of panel when task is blocked by relations */}
        {(relations?.incoming ?? []).filter(r => r.relation_type === 'blocks').length > 0 && (
          <div className="flex items-center gap-2 px-5 py-2 border-b border-red-200 bg-red-50 shrink-0 text-xs text-red-700 font-medium">
            <ArrowRight className="w-3.5 h-3.5 shrink-0" />
            Blocked by: {(relations?.incoming ?? []).filter(r => r.relation_type === 'blocks').map(r => (r.linked_task as { title: string } | undefined)?.title ?? 'another task').join(', ')}
          </div>
        )}
        ...
        <div className="flex-1 overflow-hidden flex flex-col lg:flex-row">
          ...
          <div className="flex-1 min-w-0 overflow-y-auto">
              ...
              {canManage && editingField === 'title' ? (
                ...
                <div className="flex items-start gap-2">
                  <h1
                    ...
                    className={cn('text-2xl font-bold text-slate-900 leading-tight flex-1', canManage && 'cursor-text hover:text-novax transition-colors')}
                    title={canManage ? 'Click to edit' : undefined}
                  >
                    {task.title}
                  </h1>
                  {task.is_pinned && (
                    <BookmarkCheck className="w-4 h-4 text-novax shrink-0 mt-1.5" />
                  )}
                </div>
              )}
            ...
            <div className="border-t border-slate-100 px-6 py-5 space-y-3">
              <div className="flex items-center gap-2">
                <ListChecks className="w-3.5 h-3.5 text-novax-muted" />
                <p className="text-xs font-semibold text-slate-700 uppercase tracking-wider">Checklist</p>
                {checklist.length > 0 && (
                  ...
                  {checklist.map(item => (
                    <div key={item.id} className="flex items-center gap-2.5 group">
                      <button
                        ...
                        className="shrink-0 text-slate-400 hover:text-novax transition-colors"
                      >
                        {item.done
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-1.5 rounded-lg border border-slate-200 text-xs text-slate-800 placeholder:text-slate-400 outline-none focus:border-novax-muted"
                />
                <button
                  onClick={addChecklistItem}
                  disabled={!checklistInput.trim()}
                  className="px-3 py-1.5 text-xs font-semibold bg-novax text-white rounded-lg disabled:opacity-40 hover:bg-novax-hover transition-colors"
                >
                  Add
                </button>
              </div>
          ...
          <div className="lg:w-72 shrink-0 border-t lg:border-t-0 lg:border-l border-slate-100 bg-slate-50/40 overflow-y-auto">
            <div className="p-5 space-y-5">
              ...
              {/* Watchers */}
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-2">Watchers</p>
                <div className="space-y-2">
                  {/* Watcher avatar stack */}
                  {(task.watchers ?? []).length > 0 && (
                    <div className="flex items-center gap-1 flex-wrap">
                      {(task.watchers ?? []).slice(0, 5).map(uid => {
                      ...
                      {(task.watchers ?? []).length > 5 && (
                        <span className="text-[10px] text-slate-400 font-semibold">+{(task.watchers ?? []).length - 5}</span>
                      )}
                    </div>
                  )}
                  <button
                    ...
                    className={cn(
                      'flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold border transition-colors w-full justify-center',
                      isWatching
                        ? 'bg-novax-light border-novax-border text-novax hover:bg-novax-light-hover'
                        : 'border-slate-200 text-slate-500 hover:bg-slate-50 hover:border-slate-300',
                    )}
                  >
                    {isWatching ? <BellOff className="w-3 h-3" /> : <Bell className="w-3 h-3" />}
                    {isWatching ? 'Unwatch' : 'Watch task'}
                  </button>
                </div>
              </div>
              ...
              {/* Start Date */}
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">Start Date</p>
                {canManage && editingField === 'start_date' ? (
                  <input
                    type="date"
                    value={draftStartDate}
                    autoFocus
                    onChange={e => setDraftStartDate(e.target.value)}
                    onBlur={() => { save('start_date', draftStartDate || null); }}
                    onKeyDown={e => {
                      if (e.key === 'Enter') save('start_date', draftStartDate || null)
                      if (e.key === 'Escape') { setDraftStartDate(task.start_date ?? ''); setEditingField(null) }
                    }}
                    className="text-sm border border-novax-border rounded-lg px-2 py-1 outline-none focus:border-novax-muted w-full"
                  />
                ) : (
                  <p
                    onClick={canManage ? () => { setDraftStartDate(task.start_date ?? ''); setEditingField('start_date') } : undefined}
                    className={cn('text-sm font-medium', canManage && 'cursor-pointer hover:text-novax transition-colors', task.start_date ? 'text-slate-700' : 'text-slate-400 italic')}
                  >
                    {task.start_date ? formatDate(task.start_date) : canManage ? 'Set start date' : '—'}
                  </p>
                )}
              </div>
              ...
              {/* Estimated Hours */}
              <div>
                <p className="text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-1.5">Effort Estimate</p>
                {canManage && editingField === 'estimated_hours' ? (
                  <select
                    value={draftEstHours}
                    autoFocus
                    onChange={e => { setDraftEstHours(e.target.value); save('estimated_hours', e.target.value ? parseFloat(e.target.value) : null) }}
                    onBlur={() => setEditingField(null)}
                    className="text-sm border border-novax-border rounded-lg px-2 py-1 outline-none focus:border-novax-muted w-full"
                  >
                    <option value="">No estimate</option>
                    {[0.5, 1, 2, 4, 8, 16, 24, 40].map(h => (
                      <option key={h} value={h}>
                        {h === 0.5 ? '30 min' : h === 1 ? '1 hour' : h < 8 ? `${h} hours` : h === 8 ? '1 day' : h === 16 ? '2 days' : h === 24 ? '3 days' : '1 week'}
                      </option>
                    ))}
                  </select>
                ) : (
                  <p
                    onClick={canManage ? () => setEditingField('estimated_hours') : undefined}
                    className={cn('text-sm font-medium', canManage && 'cursor-pointer hover:text-novax transition-colors', task.estimated_hours ? 'text-slate-700' : 'text-slate-400 italic')}
                  >
                    {task.estimated_hours
                      ? task.estimated_hours < 1 ? '30 min' : task.estimated_hours === 1 ? '1 hour' : task.estimated_hours < 8 ? `${task.estimated_hours}h` : task.estimated_hours === 8 ? '1 day' : task.estimated_hours === 16 ? '2 days' : task.estimated_hours === 24 ? '3 days' : '1 week'
                      : canManage ? 'Set estimate' : '—'}
                  </p>
                )}
              </div>
              ...
              <div>
                <button
                  ...
                  className="w-full flex items-center justify-between text-[10px] text-slate-400 uppercase tracking-wider font-semibold mb-2 hover:text-slate-600 transition-colors"
                >
                  <span className="flex items-center gap-1.5">
                    <Timer className="w-3 h-3" />
                    Time Logged
                    {timeLogs.length > 0 && (
                      <span className="text-novax font-bold">{totalLoggedHours.toFixed(1)}h{task.estimated_hours ? ` / ${task.estimated_hours}h` : ''}</span>
                    )}
                  </span>
                  {showTimeLogs ? <ChevronDown className="w-3 h-3" /> : <ChevronRight className="w-3 h-3" />}
                </button>
                {showTimeLogs && (
                  <div className="space-y-2">
                    {task.estimated_hours && timeLogs.length > 0 && (
                      <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                        <div
                          className={cn('h-full rounded-full transition-all', totalLoggedHours > task.estimated_hours ? 'bg-red-400' : 'bg-emerald-400')}
                          style={{ width: `${Math.min((totalLoggedHours / task.estimated_hours) * 100, 100)}%` }}
                        />
                      </div>
                    )}
                    {/* Log form */}
                    <div className="flex gap-1">
                      <input
                        ...
                        placeholder="hrs"
                        className="w-16 text-xs border border-slate-200 rounded-lg px-2 py-1 outline-none focus:border-novax-border"
                      />
                      <input
                        value={timeLogNote}
                        onChange={e => setTimeLogNote(e.target.value)}
                        placeholder="note (optional)"
                        className="flex-1 text-xs border border-slate-200 rounded-lg px-2 py-1 outline-none focus:border-novax-border"
                      />
                      <button
                        onClick={handleLogTime}
                        disabled={!timeLogHours || timeLogSaving}
                        className="px-2 py-1 text-xs font-semibold bg-novax-light border border-novax-border text-novax rounded-lg hover:bg-novax-light-hover transition-colors disabled:opacity-40"
                      >
                        Log
                      </button>
                    </div>
                    ...
                    {timeLogs.map(log => {
                      ...
                      return (
                        <div key={log.id} className="flex items-start justify-between gap-2 text-xs text-slate-600">
                          <div className="flex items-center gap-1.5">
                            {logUser && (
                              <div className="w-4 h-4 rounded-full flex items-center justify-center text-white text-[8px] font-bold shrink-0" style={{ background: logUser.color }}>{logUser.initials}</div>
                            )}
                            <span className="font-semibold">{log.hours}h</span>
                            {log.note && <span className="text-slate-400 truncate max-w-[100px]">{log.note}</span>}
                          </div>
                          <div className="flex items-center gap-1 shrink-0">
                            <span className="text-slate-400 text-[10px]">{formatDate(log.logged_at)}</span>
                            {authUser && log.user_id === authUser.id && (
                              <button
                                onClick={() => deleteTimeLog.mutate({ logId: log.id, taskId: task.id })}
              ...
              <div>
                ...
                >
                  <span className="flex items-center gap-1.5">
                    ...
                    Relations
                    {((relations?.outgoing?.length ?? 0) + (relations?.incoming?.length ?? 0)) > 0 && (
                      <span className="text-novax font-bold">{(relations?.outgoing?.length ?? 0) + (relations?.incoming?.length ?? 0)}</span>
                    )}
                  </span>
                  ...
                  <div className="space-y-2.5">
                    ...
                    {(relations?.incoming ?? []).filter(r => r.relation_type === 'blocks').length > 0 && (
                      <div className="flex items-center gap-1.5 text-xs text-red-600 bg-red-50 border border-red-200 rounded-lg px-2.5 py-1.5">
                        ...
                        Blocked by {(relations?.incoming ?? []).filter(r => r.relation_type === 'blocks').map(r => (r.linked_task as { title: string } | undefined)?.title ?? 'a task').join(', ')}
                      </div>
                    )}
                    {/* Outgoing */}
                    {(relations?.outgoing ?? []).map(r => (
                      <div key={r.id} className="flex items-center justify-between gap-2 text-xs">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className={cn('shrink-0 text-[10px] px-1.5 py-0.5 rounded font-semibold', r.relation_type === 'blocks' ? 'bg-red-50 text-red-600' : 'bg-slate-100 text-slate-500')}>
                            {r.relation_type === 'blocks' ? 'Blocks' : 'Related'}
                          </span>
                          <span className="text-slate-700 truncate">{(r.linked_task as { title: string } | undefined)?.title ?? '...'}</span>
                        </div>
                        <button
                          onClick={() => deleteRelation.mutate({ relationId: r.id, fromId: task.id, toId: r.to_task_id })}
                    ...
                    {/* Incoming */}
                    {(relations?.incoming ?? []).filter(r => r.relation_type === 'related').map(r => (
                      <div key={r.id} className="flex items-center justify-between gap-2 text-xs">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="shrink-0 text-[10px] px-1.5 py-0.5 rounded font-semibold bg-slate-100 text-slate-500">Related</span>
                          <span className="text-slate-700 truncate">{(r.linked_task as { title: string } | undefined)?.title ?? '...'}</span>
                        </div>
                      </div>
                    ))}
                    ...
                    <div className="space-y-1.5">
                      <div className="flex gap-1">
                        <select
                          value={relSearchType}
                          onChange={e => setRelSearchType(e.target.value as 'blocks' | 'related')}
                          className="text-[10px] border border-slate-200 rounded-md px-1.5 py-1 outline-none"
                        >
                        ...
                        <input
                          ...
                          className="flex-1 text-xs border border-slate-200 rounded-md px-2 py-1 outline-none focus:border-novax-border"
                        />
                      </div>
                      {relSearchResults.length > 0 && (
                        <div className="bg-white border border-slate-200 rounded-lg overflow-hidden shadow-sm">
                          {relSearchResults.map(t => (
                            <button
                              key={t.id}
                              onClick={() => {
                                createRelation.mutate({ fromId: task.id, toId: t.id, type: relSearchType })
                                setRelSearchQuery('')
                              }}
                              className="w-full text-left px-3 py-2 text-xs text-slate-700 hover:bg-novax-light transition-colors truncate border-b border-slate-100 last:border-0"
                            >
                              {t.title}
                            </button>
                          ))}
                ...
                {showActivityLog && (
                  <div className="space-y-2">
                    {activityLog.length === 0 ? (
                      ...
                      const label = (entry.action as string)
                        .replace('task.', '')
                      ...
                      return (
                        <div key={entry.id} className="flex items-start gap-2 text-xs">
                          {actor ? (
                            <div className="w-4 h-4 rounded-full flex items-center justify-center text-white text-[8px] font-bold shrink-0 mt-0.5" style={{ background: actor.color }}>{actor.initials}</div>
                          ) : (
                            <div className="w-4 h-4 rounded-full bg-slate-200 shrink-0 mt-0.5" />
                          )}
                          <div className="min-w-0">
                            <span className="text-slate-700">{actor?.name ?? 'Someone'} </span>
                            <span className="text-slate-500">{label}</span>
                            {meta?.change && <span className="text-slate-400"> · {meta.change as string}</span>}
                            <div className="text-[10px] text-slate-400 mt-0.5">{timeAgo(entry.created_at)}</div>
                          </div>
                        </div>
                      )
                    })}
